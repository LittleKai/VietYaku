...
**Session #25:** Cập nhật `.gitignore` phù hợp: (1) Loại bỏ `.claude/` khỏi danh sách ignore để cho phép theo dõi tài liệu dự án (`PROJECT_SUMMARY.md`, `CONVENTIONS.md`, v.v.), chỉ bỏ qua các file cục bộ (`*.local.json`, `*.lock`, `prompt.txt`); (2) Thay đổi `.codegraph/` thành `.codegraph/*` kết hợp với `!.codegraph/.gitignore` để theo dõi file `.gitignore` bên trong; (3) Bỏ qua các file tạm trong `/tmp/*` ngoại trừ ảnh chụp giao diện QA `/tmp/ui-qa/`; (4) Bổ sung bỏ qua `.flutter-plugins`, `.packages` và các file từ điển đã sửa đổi phát sinh `**/*_JP.txt`.
