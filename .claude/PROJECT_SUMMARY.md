...
└── test/                           # 518 tests (59 file; integration dữ liệu thật tự skip nếu thiếu path)
```
...
### Data Flow
settings (paths) → dictionaries_provider → DictionaryRepository.loadAll (12 dict chính + UserNames local + 2 shared overlay, tải song song qua `Isolate.run`; cache `.vydc` hợp lệ → decode, không thì parse text + ghi cache) → mode Nhật + Sudachi bật: `SudachiVariantGroups` (nạp song song với các dict) mở rộng UserDict/UserNames/shared VietPhrase/shared Lạc Việt bằng alias kana/kanji an toàn — nhóm được cắt đuôi kana chung nên entry dạng thân từ (`吞み込`) cũng khớp; key khớp trọn một surface nhận cả nhóm, còn khi ghép nhiều mảnh thì mảnh phụ chỉ được kana hoá; alias KHÔNG bao giờ bắc cầu giữa chữ Hán và katakana, cũng không dựng kanji từ key thuần kana (`細工` không sinh `ザイク`, `ザイク=Zaik` không sinh `細工=Zaik`) (entry ghi rõ vẫn thắng; alias thuần kana ngắn chỉ nhận khi key ≥ 3 đơn vị, nên `吞み込` có `のみこ` còn `空` không sinh `から`) → shared VietPhrase/Lạc Việt đè entry cùng key trong file bundle → LoadedDictionaries.engineWith(algorithm, prioritizeNames) → translation_controller.translate → tokens + hanVietTokens → TokenTextView: nháy chuột → lookup; chuột phải không tô đen → paste nghĩa vào ô Bản dịch nhưng không đổi active/highlight; chuột phải khi tô đen → admin sửa VietPhrase/Lạc Việt cục bộ và xếp hàng Update, non-admin sửa UserDict, Names luôn local.
...
| Sửa từ điển từ toolbar chuột phải | ✅ Done | source_pane, token_text_view, icon_context_menu, entry_edit_dialog, user_dict_service | Biên nguồn vùng chọn, Hán Việt chip, auto-expand, tier colors, focus active highlights; đổi ô Từ nguồn thì Nghĩa + Hán Việt + trạng thái Glossary + preview tác động nạp lại theo key mới (giữ nguyên nếu người dùng đã tự sửa ô Nghĩa); nút Xóa từ hiện/ẩn theo key hiện tại (hỗ trợ cả Sửa nghĩa UserDict và Thêm vào Names — xóa khỏi file local UserNames.txt/UserDict.txt và tùy chọn xóa cả Global Glossary nếu có, kèm dialog xác nhận phạm vi xóa). Toàn bộ thao tác Glossary nằm trọn trong thẻ trạng thái Glossary (nút **Thêm vào Glossary** khi chưa có; khi đã có và tắt tự động cập nhật thì hiện 2 nút mang màu sắc khác nhau: **Cập nhật theo VietPhrase** màu cyan đứng trước và **Sửa nghĩa Glossary** màu purple đứng sau) |
