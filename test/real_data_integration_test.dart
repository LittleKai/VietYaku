...
void main() {
  ...
  final available =
      vietPhraseFile.existsSync() &&
      namesFile.existsSync() &&
      phienAmFile.existsSync();

  group('real data integration', () {
    test(
      'parses real dictionaries and translates a Japanese passage',
      () async {
        final sw = Stopwatch()..start();
        final vietPhrase = parseDictionary(
          await vietPhraseFile.readAsString(),
          DictType.vietPhrase,
        );
        final names = parseDictionary(
          await namesFile.readAsString(),
          DictType.names,
        );
        final phienAm = parseDictionary(
          await phienAmFile.readAsString(),
        ...
        // ignore: avoid_print
        print(
          'parse 3 dicts: ${sw.elapsedMilliseconds}ms; '
    ...
    test(
      'translate 10k characters in under 200ms',
      () async {
        final vietPhrase = parseDictionary(
          await vietPhraseFile.readAsString(),
          DictType.vietPhrase,
        ...
        // ignore: avoid_print
        print(
          'translate ${text.length} chars: ${sw.elapsedMilliseconds}ms, '
    ...
    test(
      '.vydc cache: cold parse vs warm load on all 5 real files',
      () {
        ...
        for (final entry in files.entries) {
          ...
          // ignore: avoid_print
          print(
            '${entry.value}: cold=${cold.elapsedMs}ms '
