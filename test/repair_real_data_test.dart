...
void main() {
  ...
  group('repair trên dữ liệu thật', () {
    test('LacViet.txt: hết key kana bị chèn space, value nguyên vẹn', () async {
      final content = await lacVietFile.readAsString();
      final sw = Stopwatch()..start();
      final result = repairFile(content, table, RepairPolicy.addVariant);
      sw.stop();
      // ignore: avoid_print
      print('LacViet repair: ${sw.elapsedMilliseconds}ms — '
          '${result.report}');
      ...
      for (final entry in repaired.entries) {
        expect(originalValues.contains(entry.value), isTrue,
            reason: 'value lạ (bị biến đổi) ở key ${entry.key}');
      }
    }, skip: available ? false : 'thiếu dữ liệu thật');

    test('VietPhrase.txt: repair + nạp lại → match dài hơn', () async {
      ...
      final before =
          parseDictionary(content, DictType.vietPhrase);
      final after =
      ...
      // ignore: avoid_print
      print('matched units: before=$unitsBefore after=$unitsAfter '
